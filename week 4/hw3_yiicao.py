# -*- coding: utf-8 -*-
"""“SchellingAssignment (2).ipynb”的副本

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1SARKReXSP1ODbYCoHiOFADtOwGnwyJxG
...
agent2 = Agent(66, 88) 
map_an_agent(agent2)####call the function to map this agent
...
def map_all_agents(agents_list):  # what arguments should go in this function?
...
map_all_agents(agents_list)##Use the function we just created to map our agents
...
def make_agents_dance(agents_list, number_of_steps):
...
def map_colorful_agents(List_of_Agents):  # what argument should go in this function?

    Purple_XCoordinate = [agent.xlocation for agent in List_of_Agents if agent.group=="Purple"] ####fill in the blanks to make this list comprehension work
    Purple_YCoordinate = [agent.ylocation for agent in List_of_Agents if agent.group=="Purple"] #model this after the above list comprehension
    Gold_XCoordinate = [agent.xlocation for agent in List_of_Agents if agent.group=="Gold"]
    Gold_YCoordinate = [agent.ylocation for agent in List_of_Agents if agent.group=="Gold"]
...
map_colorful_agents(CombinedList)####call the function appropriately
...
print(a55.xlocation)  ### if your class is set up right the printed answer should be 81 not 24
...
class AgentNew(Agent):
    ...
    def check_neighbors(self, agentlist):  # note this method needs not only the self attributes - it also needs a list of agents
        ...
        zlist = list(filter(lambda q: (self.xlocation-10) < q.xlocation < (self.xlocation+10), agentlist))##### here build use a lambda function just as in the prior cell to find all agents in a list who are within 10 spaces in the X direction of the agent that is calling the method
        zlist = list(filter(lambda q: (self.ylocation-10) < q.ylocation < (self.ylocation+10), zlist))###### do the same thing now to filter for agents within 10 spaces in the y dimension. what list do you want to apply this lambda function to?
        same_group_neighbor = [agent for agent in zlist if self.group == agent.group]#### use list comprehension to only keep members of zlist who are of the same group as this agent
        opposite_group_neighbor = [agent for agent in zlist if self.group != agent.group]#### use list comprehension to only keep members of zlist who are of the opposite group as this agent
...
testlist = [PurpleAgents(random.randint(0,100), random.randint(0,100)) for i in range(400)] + [GoldAgents(random.randint(0,100), random.randint(0,100)) for i in range(400)] ###### create a testlist that has 200 PurpleAgents and 200 GoldAgents
map_colorful_agents(testlist)
for x in range(15):
    for agent in (testlist):
        agent.check_neighbors(testlist)  #does this need any arguments?
...
class GoldDiversitySeekers(Agent):
    ...
    def check_neighbors(self, agentlist):  # note this method needs not only the self attributes - it also needs a list of agents
        ...
        if (len(same_group_neighbor)+.01)/(len(zlist)+.01) < group_affinity_threshold: #this command works. It checks the percentage of same group neighbors against some threshold to determine the agents happyness.
            ...
            self.status="unhappy"
random.seed(11)
testlist1 = [PurpleAgents(random.randint(0,100), random.randint(0,100)) for i in range(300)] + [GoldAgents(random.randint(0,100), random.randint(0,100)) for i in range(300)]
group_affinity_threshold = .51
map_colorful_agents(testlist1)
for x in range(15):
    for agent in (testlist1):
        agent.check_neighbors(testlist1)  #does this need any arguments?
    for agent in (testlist1):
        agent.move_if_unhappy() #does this need any arguments?
    map_colorful_agents(testlist1)
    print(x)
...
testlist2 = [PurpleDiversitySeekers(random.randint(0,100), random.randint(0,100)) for i in range(100)] + [GoldDiversitySeekers(random.randint(0,100), random.randint(0,100)) for i in range(100)]
group_affinity_threshold = .51
map_colorful_agents(testlist2)
for x in range(15):
    for agent in (testlist2):
        agent.check_neighbors(testlist2)  #does this need any arguments?
    for agent in (testlist2):
        agent.move_if_unhappy() #does this need any arguments?
    map_colorful_agents(testlist2)
    print(x)
