...
def make_move(player, board):
    ...
    displays who's move it is (X or O) #print()
    prompts the user to enter a number 1-9 #input()
    validates input, repeats until valid input is entered #while()
    ...
    print("Player {}".format(player_name(player)))
    #2. prompt use
    while True:
        try:
            move = int(input("Where do you want to move. Enter a number 1-9 "))
            move -= 1
...
def check_win_horizontal(board):
    '''checks if a player wins horizontally

    checks if any row is occupied by the same player
...
def check_win_vertical(board):
    # TODO: write docstring
    '''checks if a player wins vertically

    checks if any column is occupied by the same player
...
def check_win_diagonal(board):
    # TODO: write docstring
    '''checks if a player wins diagonally

    checks if any diagonal is occupied by the same player
